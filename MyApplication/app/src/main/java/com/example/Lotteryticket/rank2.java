...
public class rank2 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        for(int i=1;i<7;i++){
            ...
            if(i==1){
                textViews[i-1].setText("사용금액 :    "+formatter.format(ints[i-1])+" 원");
            }
            else{
                if(i==2){
        ...
        imageView.setOnTouchListener(new View.OnTouchListener() {
            public boolean onTouch(View v, MotionEvent event) {
                switch (v.getId()) {
                    case R.id.imagebtn3:
                        Intent intent=new Intent(rank2.this,MainActivity2.class);
        ...
        imageView2.setOnTouchListener(new View.OnTouchListener() {
            public boolean onTouch(View v, MotionEvent event) {
                switch (v.getId()) {
                    case R.id.imagebtn2:
                        Intent intent=new Intent(rank2.this,MainActivity.class);
                        startActivity(intent);
                        finish();
                        break;
                }
